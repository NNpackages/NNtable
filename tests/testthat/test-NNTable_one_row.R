...
test_that("Bug one_row rename test 1", {

  .NNTable <- adsl_f[1, , drop = FALSE] %>%
...
test_that("Bug one_row rename test 4", {

  .NNTable <- adsl_f[1, , drop = FALSE] %>%
    mutate(order = as.factor(SUBJID)) %>%
    NNTable(cols[1], page_size = getEOTpaper("land", 9)) %>%
    #addGroupedColumns(names(cols)[1], name = names(cols)[1]) %>%
    addOrder(order = 1) %>%
    addCellSplit(align = "bottom") %>%
    addAlignment("Exposed in treatment" = "l", "period (Y/N)/" = "l") %>%
...
test_that("Bug one_row rename test 5", {
