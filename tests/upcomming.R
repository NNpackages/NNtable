...
table <- NNTable(adpc, Sex = "sex", "cohort", "TRTP", "MEAN","MIN", "MAX", page_size = list(page.length = 81, page.width = 120)) %>%
...
table <- NNTable(adpc, Sex = "sex", "cohort", "TRTP", "MEAN","MIN", "MAX", page_size = list(page.length = 81, page.width = 200)) %>%
  addTransLong(SUM = c("MEAN", "MIN" , "MAX"), var_name = "Summary") %>%
  addTransWide(cohort = list(TRTP = "SUM")) %>%
  addUnderScore() %>%
  addGroupedColumns("Summary") %>%
